class PlaidController < ApplicationController
  ...
  # POST /plaid/exchange
  # Body: { public_token: 'public-...' }
  # Exchanges the public_token for a permanent access_token, discovers the Amex
  # account, and upserts the provider='plaid' bank_connection row (name='amex').
  def exchange
    user = current_user
    return render json: { error: 'Unauthorized' }, status: :unauthorized unless user
    ...
    begin
      ...
      # On first connect, start Plaid where Teller left off. Plaid's backfill can
      # reach ~24 months; since Plaid txn ids differ from Teller's (no dedupe),
      # we cut over at the last Amex txn already in the DB. Only set on create —
      # never override an existing cutoff on re-connect.
      if connection.sync_from_date.nil?
        connection.sync_from_date = FinancialTransaction.where(source: 'amex').maximum(:transacted_at)&.to_date
      end
      connection.save!
